...
public class RequestHandler implements Runnable {
	...
	public void run() {
		...
		System.out.println("Pedido recebido " + requestString);
		...
		if(Proxy.isBlocked(urlString)){
			System.out.println("Pedido de site bloqueado : " + urlString);
		...
		if(request.equals("CONNECT")){
			System.out.println("Pedido HTTPS para : " + urlString + "\n");
			...
			if((file = Proxy.getCachedPage(urlString)) != null){
				System.out.println("Copia em cache de : " + urlString + "\n");
				sendCachedPageToClient(file);
			} else {
				System.out.println("HTTP GET para : " + urlString + "\n");
	...
	private void sendCachedPageToClient(File cachedFile){
		...
		try{
			...
			System.out.println("Erro enviando arquivo em cache para o cliente");
			e.printStackTrace();
		}
	}
	...
	private void sendNonCachedToClient(String urlString){
		...
		try{
			...
			try{
				...
				System.out.println("Nao foi possivel salvar: " + fileName);
				caching = false;
				e.printStackTrace();
			...
			if((fileExtension.contains(".png")) || fileExtension.contains(".jpg") ||
				...
				// No image received from remote server
					...
					System.out.println("Enviando 404 para o cliente pois a imagem nao foi recebida do servidor"
							+ fileName);
	...
	class ClientToServerHttpsTransmit implements Runnable{
		 ...
		 * proxyToClientIS Stream that proxy uses to receive data from client
		 * proxyToServerOS Stream that proxy uses to transmit data to remote server
		 */
		...
		public void run(){
			try {
				...
				System.out.println("TIMEOUT de leitura");
				e.printStackTrace();
			}
		}
	}
	private void blockedSiteRequested(){
		try {
			...
			System.out.println("Erro ao enviar para o cliente pedido de site bloqueados");
			e.printStackTrace();
		}
	}
}
